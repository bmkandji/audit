...
% =====================================================================
\begin{itemize}
\item \textbf{Pas et fenêtre.} $h=\Dt=1/12$ (mensuel), $n\approx240$ (fenêtre 20~ans). Dates $t,t+h,\dots$
\item \textbf{Régime commun.} $\Ecal_t\in\{1,\dots,K\}$, chaîne de Markov de matrice $P=(p_{ab})$, $p_{ab}=\Prob(\Ecal_{t+h}=b\mid\Ecal_t=a)$, de loi invariante $\pi$ ($\pi\transp P=\pi\transp$).
\item \textbf{Facteurs.} Indexés par $i$~; état $Y^{(i)}_t\in\R^{d_i}$ ($d_i=2$ pour un Vasicek bi-factoriel V2F, $1$ sinon)~; état global $Y_t\in\R^{d}$, $d=\sum_i d_i$.
\item \textbf{Innovations.} $\eps_{t+h}\in\R^d$, \emph{standardisées} (marginalement $\Ncal(0,1)$), de matrice de corrélation \emph{pleine, libre, propre au régime} $\Omega(\Ecal_t)$~; $\eps_{t+h}=L(\Ecal_t)z_{t+h}$, $z\sim\Ncal(0,I_d)$, $L(a)L(a)\transp=\Omega(a)$.
\item \textbf{Lois.} $\Phi,\varphi$~: f.d.r.\ et densité $\Ncal(0,1)$~; $\varphi_p(\cdot;m,V)$~: densité $\Ncal(m,V)$~; $f_i$~: densité de transition exacte du facteur~$i$, $F_i$ sa f.d.r.~; $F_{\chi'^2}(\cdot;\nu,\lambda)$~: f.d.r.\ d'un $\chi^2$ décentré.
\item \textbf{Estimation.} $\hat\theta_i$~: paramètres de marge~; $\ell$~: log-vraisemblance.
\end{itemize}
...
Les facteurs du Groupe~B partagent le régime \emph{commun} $\Ecal_t$~; ils s'estiment \emph{conjointement} par EM.
\paragraph{Étape E.} \emph{Filtre de Hamilton} (avant, $f_a(x_t)=\varphi_p(x_t;m_a,V_a)$) puis \emph{lisseur de Kim} (arrière), donnant les probabilités lissées $\hat\xi_t(a)=\Prob(\Ecal_t=a\mid\Xcal_n)$ et de transition $\hat\xi_t(a,b)$.
...
\]
$\hat V_a$ \emph{pleine} (corrélations intra-Groupe~B par régime estimées). Ascension monotone garantie~; planchers de variance contre les maxima dégénérés~; tri des états par volatilité décroissante (régime~1 = stress).

\subsection{Étape 3 --- dépendance pleine, par régime}
\label{sec:etape3}
Marges et $\hat\xi_t(a)$ figées, soit $\widehat\eps_t$ le résidu standardisé \emph{complet}. À marges fixées, la corrélation empirique \emph{pondérée par le régime} maximise la log-vraisemblance de la copule gaussienne~:
\begin{definitionbox}
\[
\widehat\Omega(a)=\frac{\sum_t\hat\xi_t(a)\,\widehat\eps_t\widehat\eps_t\transp}{\sum_t\hat\xi_t(a)},\qquad a=1,\dots,K^\star .
\]
\end{definitionbox}
\noindent En échantillon fini, $\widehat\Omega(a)$ d'un régime peu peuplé peut être quasi singulière~: on applique un \emph{rétrécissement} (shrinkage) vers la cible poolée, d'intensité fonction de la taille d'échantillon \emph{effective} du régime, puis la projection sur le cône SDP le plus proche (Higham), \emph{régime par régime}. Toutes les entrées (intra-V2F, intra-Groupe~B, croisées) sont estimées, jamais imposées. Un masque de sensibilité $S$ permet de rendre certaines corrélations communes aux régimes (\S\ref{sec:algo}).
...
La discrétisation est un VAR(1) gaussien $Y_{t+\Dt}=c(\theta)+\Phi(\theta)Y_t+\eta$, $\eta\sim\Ncal(0,\Sigma_{\mathrm{cond}}(\theta))$, $\Phi(\theta)=e^{-A\Dt}$, $\Sigma_{\mathrm{cond}}(\theta)$ par Lyapunov, fonctions \emph{exactes} de $\theta=(\kappa_1,\kappa_2,\sigma_1,\sigma_2,\rho,\mu)$.
\begin{itemize}[nosep]
\item \texttt{mle} \emph{(défaut)} --- \textbf{EMV joint pur}, court \& long \emph{ensemble}, sur le domaine admissible ($\kappa_1>\kappa_2>0$, $\sigma_i>0$)~:
\[
\hat\theta=\argmax_\theta\sum_t\log\varphi_2\big(Y_{t+\Dt};c(\theta)+\Phi(\theta)Y_t,\Sigma_{\mathrm{cond}}(\theta)\big).
\]
Aucun passage par une régression ni back-out implicite~; $\mu$ fixé (cible) ou estimé.
...
\]
($B_2$ est l'\emph{intégrale} du coefficient de transmission $\phi_{12}$ de \S\ref{sec:v2f}), $B_{ik}=B_k(\tau_i)/\tau_i$. Avec $\Sigma^X_\infty$ (Lyapunov), $\Sigma^{\Delta X}_\infty=2\Sigma^X_\infty-\Phi\Sigma^X_\infty-\Sigma^X_\infty\Phi\transp$, et $\Sigma^Y=B\Sigma^X B\transp$, $\Sigma^{\Delta Y}=B\Sigma^{\Delta X}B\transp$, on ajuste trois cibles (volatilité des variations $\sqrt{(\Sigma^{\Delta Y})_{ii}/\Dt}$, corrélation inter-maturités, écart-type des niveaux $\sqrt{(\Sigma^Y)_{ii}}$) à leurs analogues historiques par moindres carrés relatifs pondérés. \textbf{Mal posé.} Le chargement court $B_1(\tau)/\tau\sim1/(\kappa_1\tau)\to0$ quand $\kappa_1$ croît~: le facteur court \emph{se découple}, l'objectif présente une crête $\sigma^2/\kappa$~; on la lève par une \emph{régularisation ridge faible}. C'est la méthode de la référence. \emph{(Résidus/simulation~: facteurs identifiés aux taux observés, comme \texttt{mle}/\texttt{ols}~; l'estimation distributionnelle ne fournit que les paramètres de marge.)}

\paragraph{(C) BS (PE/immo/infra).} Log-rendements i.i.d.\ $\Ncal(m,v)$~: $\hat m=\tfrac1n\sum x_t$, $\hat v=\tfrac1n\sum(x_t-\hat m)^2$, $\hat\sigma=\sqrt{\hat v/\Dt}$. En cadre \emph{monde réel}, la dérive retenue est la moyenne annualisée des log-rendements $\hat\mu=\hat m/\Dt$ (sans terme de convexité $\tfrac12\sigma^2$). Moyenne sur rendements bruts, volatilité sur rendements déslissés.
...
\]
puis maximisation de $\sum_t\log f_{\mathrm{CIR}}$ (variante \texttt{ols}~: init Euler seule).
...
\midrule
V2F (taux) & \texttt{mle}/\texttt{ols}/\texttt{distrib.} & EMV joint~; MCO cascade~; cibles distributionnelles\\
...
\item \textbf{Régime.} Initialiser $\Ecal_0\sim\hat\pi$~; à chaque pas, tirer $\Ecal_{t+\Dt}\mid\Ecal_t\sim\hat P_{\Ecal_t,\cdot}$ \emph{avant} le rendement (convention de l'estimation).
\item \textbf{Chocs.} $z\sim\Ncal(0,I_d)$, $\eps_{t+\Dt}=L(\Ecal_t)z$ (copule gaussienne)~; en option, copule de \emph{Student} (scatter $\Omega(\Ecal_t)$, marges restituées $\Ncal(0,1)$).
...
\midrule
\rowcolor{nxlightblue} Inflation, taux réels & 2 & A & EMV joint (mle~; ols/distrib.\ en option) / récurrence gaussienne exacte\\
...
\item \textbf{\textsc{NearestCorr}.} Projections alternées de Higham, par régime, après rétrécissement.
