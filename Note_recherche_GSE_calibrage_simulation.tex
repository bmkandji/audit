...
\label{sec:notations}
% =====================================================================
Nous fixons ici les notations employées dans toute la note. Le temps est discrétisé au pas mensuel $\Dt=1/12$, sur une fenêtre d'environ $n\approx240$ observations (vingt ans). Nous indexons les facteurs par $i$ et les régimes par $a,b\in\{1,\dots,K\}$. L'état global empile tous les facteurs en un vecteur $Y_t\in\R^{d}$ de dimension $d=\sum_i d_i$ (ici $d=12$), chaque facteur $i$ ayant un état $Y^{(i)}_t\in\R^{d_i}$ ($d_i=2$ pour le Vasicek bi-factoriel, $1$ sinon)~; les actions, qui portent le régime, sont au nombre de $D=3$.

L'\textbf{état économique} est résumé par une chaîne de Markov commune $\Ecal_t\in\{1,\dots,K\}$, de matrice de transition $P=(p_{ab})$ où $p_{ab}=\Prob(\Ecal_{t+\Dt}=b\mid\Ecal_t=a)$. Sa loi invariante $\pi$ vérifie $\pi\transp P=\pi\transp$ et donne les probabilités stationnaires des régimes. Lorsqu'on estime ce régime, $\xi_{t\mid t}(a)$ désigne la probabilité \emph{filtrée} d'être dans le régime $a$ (au vu du passé) et $\hat\xi_t(a)$ la probabilité \emph{lissée} (au vu de tout l'échantillon)~; $\hat\xi_t(a,b)$ est la probabilité lissée de transition (\S\ref{sec:calib}).

Les \textbf{innovations} $\eps_{t+\Dt}\in\R^d$ sont standardisées (chaque composante est marginalement $\Ncal(0,1)$) et corrélées selon une matrice $\Omega(\Ecal_t)$ propre au régime~; on les obtient par $\eps=L(\Ecal_t)z$, où $z\sim\Ncal(0,I_d)$ et $L(a)$ est le facteur de Cholesky de $\Omega(a)$. La récurrence de chaque facteur fait intervenir une matrice autorégressive $\Phi_i$, une constante $c_i$ et une échelle de bruit $D_i$. Pour le Vasicek bi-factoriel en particulier, $\Phi=e^{-A\Dt}$, $\phi_{12}$ mesure le couplage du court vers le long, $\Sigma_{\mathrm{cond}}=\bigl(\begin{smallmatrix}V_r&C_{mr}\\C_{mr}&V_m\end{smallmatrix}\bigr)$ est la covariance conditionnelle (de variances $V_r,V_m$ et de covariance $C_{mr}$), et $\rho_c=C_{mr}/\sqrt{V_rV_m}$ la corrélation conditionnelle.

Côté \textbf{paramètres de marge}, le Vasicek bi-factoriel est décrit par les vitesses de retour à la moyenne $\kappa_1,\kappa_2$ (court et long), les volatilités $\sigma_1,\sigma_2$, la corrélation $\rho$ de ses browniens et la cible long terme $\mu$~; le CIR par $(\kappa,\theta,\sigma)$ et les paramètres $c_\chi,\nu,\lambda$ de sa loi $\chi^2$ décentrée~; le Black--Karasinski par $(k,\theta,\sigma)$~; les facteurs Black--Scholes (et Hardy par régime) par un drift et une volatilité $(\mu_a,\sigma_a)$, dont on déduit les moments par pas $m_a=(\mu_a-\tfrac{\sigma_a^2}{2})\Dt$ et $v_a=\sigma_a^2\Dt$. Enfin, $f_i$ et $F_i$ désignent la densité et la fonction de répartition de transition exacte du facteur $i$, $\Phi$ et $\varphi$ celles de la loi $\Ncal(0,1)$, $\varphi_p(\cdot;m,V)$ la densité $\Ncal(m,V)$ et $F_{\chi'^2}(\cdot;\nu,\lambda)$ la f.d.r.\ du $\chi^2$ décentré~; on note $\hat\theta$ les paramètres estimés, $\ell$ la log-vraisemblance et $p_K$ le nombre de paramètres libres servant au critère d'information.
...
\]
où $\Phi=e^{-A\Dt}=\bigl(\begin{smallmatrix}e^{-\kappa_1\Dt}&\phi_{12}\\0&e^{-\kappa_2\Dt}\end{smallmatrix}\bigr)$, $\phi_{12}=\tfrac{\kappa_1}{\kappa_1-\kappa_2}(e^{-\kappa_2\Dt}-e^{-\kappa_1\Dt})$, et $\Sigma_{\mathrm{cond}}=\Sigma_\infty-\Phi\Sigma_\infty\Phi\transp$ avec $\Sigma_\infty$ solution de l'équation de Lyapunov $A\Sigma_\infty+\Sigma_\infty A\transp=\Sigma_w$.
\end{definitionbox}
\noindent La corrélation conditionnelle $\rho_c=(\Sigma_{\mathrm{cond}})_{12}/\sqrt{(\Sigma_{\mathrm{cond}})_{11}(\Sigma_{\mathrm{cond}})_{22}}$ demeure libre, car elle est portée par $\Omega$ et non fixée. Les taux réels suivent la même structure, en remplaçant $(q^s,q^l)$ par $(r,l)$.
...
\end{definitionbox}
\noindent La dépendance repose ainsi sur deux véhicules, dont aucun n'est imposé. La chaîne $\Ecal_t$ capte le co-mouvement systémique, c'est-à-dire l'entrée conjointe en stress, tandis que la matrice $\Omega(\Ecal_t)$ capte la dépendance intra-régime, sans contrainte ni a priori. Le rattachement d'un facteur au Groupe~B relève d'un choix de configuration.
...
L'estimation et la simulation partagent une même copule par régime, ce qui en assure la cohérence. Aucune corrélation n'est imposée. La chaîne commune reproduit l'entrée conjointe en stress, donc la dépendance de queue, tandis que le choix de $K^\star$ assure la parcimonie. Enfin, les estimateurs restent sur le domaine admissible et les corrélations sont rétrécies sur des résidus stationnaires, ce qui confère robustesse au dispositif.
\end{mabox}
